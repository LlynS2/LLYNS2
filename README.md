<div align="right" >

   [`Português`](https://github.com/LlynS2/LLYNS2/tree/Português)
   [`English`](https://github.com/LlynS2/LLYNS2)
   [`Español`](https://github.com/LlynS2/LLYNS2/tree/Español)
   [`中文`](https://github.com/LlynS2/LLYNS2/tree/中文)
   
</div><br>

<div align="center" >
      <a href="#" >
         <img width="1262" height="421" alt="Hello World" src="https://github.com/user-attachments/assets/9d35ad6c-9929-4994-9e86-8ae742a29880" />
      </a>
   </div>
</head><br>

<body>
   <div>
      <h2>
         <a href="#" >
            <img width="200" height="43" alt="About Me" src="https://github.com/user-attachments/assets/c500a7fe-2a09-4b72-9e3d-7d30b6953290" />
         </a>
      </h2>
      <text align="left">
         <p>
           저는 XTRHYLL 생태계의 창립자이자 CEO인 Hevellyn Mc’Frei입니다. XTRHYLL은 기술, 비즈니스, 연구를 통합하여 글로벌 및 사회적 영향 과제를 해결하기 위한 솔루션을 만드는 혁신 중심의 딥테크 기업입니다.
           저는 FIAP에서 시스템 분석 및 개발을 전공하고, USP에서 소프트웨어 공학 MBA를 취득했습니다. 저의 주요 업무는 보안, 혁신, 발전을 이끄는 기술 제품을 개발하여 더 스마트한 미래를 만드는 것입니다.
           이곳에서는 저의 여정을 형성한 개인 프로젝트, 학술 연구, 그리고 해커톤 도전들을 만나보실 수 있습니다.
         </p>
         <p>
           저는 지식이란 끊임없는 혁명이며 — 혁신, 영향력, 그리고 세상의 진정한 변화를 이끄는 핵심 동력이라고 믿습니다.
         </p>
      </text>

   <div align="right">
      <h5><a href="#" ><img width="260" height="38" alt="Professional Links" src="https://github.com/user-attachments/assets/06527d8b-6f4f-4b1c-a233-0070865f2916" /></a></h5>
   
   [`L I N K E D I N`](www.linkedin.com/in/hevellyn-mc-frei-mba-079020219) 
   [`O R C I D`](https://orcid.org/0009-0005-3356-7286)
   [`L A T T E S`](http://lattes.cnpq.br/9898418408120334)
   
   </div>
   
   </div><br>
   
   <div align="left">
      <h2>
         <a href="#" >
            <img width="200" height="43" alt="Projects" src="https://github.com/user-attachments/assets/93b81dc6-a51c-414f-a744-a56731dd76bf" />
         </a>
      </h2>
   </div><br>
   
   <div align="center">
      
   [`학 술 프 로 젝 트`](#)
   [`개 인 프 로 젝 트`](#)
   [`해 커 톤 챌 린 지`](#)
   
   </div><br>
   
   <div align="left">
      <h2>
         <a href="#" >
            <img width="350" height="43" alt="Skills and Knowledge" src="https://github.com/user-attachments/assets/8b0f9a2d-a8e2-41d1-a5f5-90d58bb226e1" />
         </a>
      </h2><br>
      <text>
         <p>
           “나는 어떤 것을 이해했다고 해서 결코 완전히 만족하지 않는다. 아무리 깊이 이해했다고 해도, 나의 이해는 내가 알고자 하는 모든 것의 극히 작은 일부일 뿐이다.”<br>
      
   `에 이 다  러 브 레 이 스`
         </p>
      </text><br>
      <table align="center">
         <tr align="left">
           <td>
             <h5>
                인공지능
             </h5>
            </td>
            <td>
               <h5>
                  머신러닝
               </h5>
            </td>
            <td>
               <h5>
                  디지털 전환
               </h5>
            </td>
            <td>
               <h5>
                  소프트웨어 공학
               </h5>
            </td>
            <td>
               <h5>
                  풀스택 개발
               </h5>
            </td>
            <td>
               <h5>
                  DevOps
               </h5>
            </td>
            <td>
               <h5>
                  사이버 보안
               </h5>
            </td>
            </tr>
            <tr align="left">
               <td>
                  <h5>
                     비즈니스 전략
                  </h5>
               </td>
               <td>
                  <h5>
                     프로젝트 관리
                  </h5>
               </td>
               <td>
                  <h5>
                     리스크 관리
                  </h5>
               </td>
               <td>
                  <h5>
                     기업 거버넌스
                  </h5>
               </td>
                  <td>
                     <h5>
                        IT 거버넌스
                     </h5>
                  </td>
                  <td>
                     <h5>
                        UI/UX 디자인
                     </h5>
                  </td>
                  <td>
                     <h5>
                        문제 해결
                     </h5>
                  </td>
            </tr>
         </table>
   </div><br>
   
   <div align="left">
      <h2 align="center">
         <a href="https://github.com/XTRHYLL">
            <img width="370" height="44" alt="XTRHYLL Corp" src="https://github.com/user-attachments/assets/06642e33-2f9d-44f1-80a9-450c96499821" />
         </a>
      </h2>
      <text>
         <p align="left" >
           <a href="https://github.com/XTRHYLL"><img  align="right" width="290" height="310" alt="XTRHYLL Logo" src="https://github.com/user-attachments/assets/0d90be3d-82db-42de-94be-54e79f74810d" /></a><br><br>
            
   [`X T R H Y L L  C O R P O R A T I O N`](https://github.com/XTRHYLL) 
           은(는) 변화의 추진, 기준의 향상, 그리고 기술·혁신·비즈니스·사회·미래를 연결하는 솔루션을 개발하기 위해 설립되었습니다.
           우리는 글로벌 임팩트를 창출하고 세상을 변화시키는 지능형 기술과 첨단 솔루션을 만드는 데 집중하고 있습니다.<br><br>
           — 대체하지 않고, 언제나 더해간다.
         </p> 
      </text>
</body><br><br>

<footer>
   <div align="center">
               
   `인공지능`
   `소프트웨어 공학`
   `글로벌 딥테크 & 혁신 생태계`
   `전략적 비즈니스`
   
   </div><br>
   <div align="center" >
      <h6>©2026  Hevellyn Mc'Frei • X T R H Y L L </h6>
   </div>
</footer>
